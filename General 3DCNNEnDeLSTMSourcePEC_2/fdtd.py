...
# Yee-Grid
class Grid(object):
	def __init__(self, dim=64, epsr=1, sigma=1e10, radius=16, obj_type="circle" ):
		self.height = dim
		self.width = dim
		...
		self.total_field_range = np.arange(self.ha + 1, self.hb)#np.arange(0, dim)#
		...
		if self.obj_type == "circle":
			...
			for j in range(self.wa, self.wb):
				for i in range(self.ha, self.hb):
					...
					if ((hc - self.radius) < i) and (i < (hc +self.radius)) \
							and ((wc - self.radius) < j) and (j < (wc + self.radius)):
						# lossy material
						self.gaz[i, j] = 1 / (self.epsr + (self.sigma * dt / epsz))
						self.gbz[i, j] = (self.sigma * dt / epsz)
	...
	def update(self, time_step):
		...
		self.train_data = [
			...
			#self.dz[np.ix_(self.total_field_range, self.total_field_range)],
		   	#np.abs(self.hx[np.ix_(self.total_field_range, self.total_field_range)]),
		   	#np.abs(self.hy[np.ix_(self.total_field_range, self.total_field_range)]),
...
ax1 = plt.subplot(111)
data_src = Grid(param.opt.n_domain_dim, epsr=1, sigma=1e4, radius=5, obj_type="circle")
...
for prepare_index in range(150):#50-150
	...
	#ax1.clear()
	#ax1.plot(input_data[2][32, :])#(data_src.ez_inc)#
	#plt.ylim([-1,1])
	#plt.pause(0.01)
