...
# Yee-Grid
class Grid(object):
	def __init__(self, dim=64, epsr=1, sigma=1000, radius=16):
		self.height = dim
		self.width = dim
...
ax1 = plt.subplot(111)
data_src = Grid(param.opt.n_domain_dim, epsr=1, sigma=1e3, radius=16)
for prepare_index in range(200):#30
	data_src.update(prepare_index)
	input_data = data_src.train_data
	ax1.clear()
	ax1.imshow(input_data[0])#(data_src.ez_inc)#
	plt.pause(0.001)
plt.show()